// ==========================================================================
// Phoenix v108.0 "Chimera" — Wokwi Integration Test (Shared Core)
//
// Single source of truth for both PlatformIO/Arduino and ESP-IDF builds.
// Platform-specific behavior is controlled by preprocessor defines:
//   PHOENIX_PLATFORM_ARDUINO  — enables Serial.printf debug output
//   PHOENIX_PLATFORM_ESPIDF   — pure ESP_LOG output only
//
...
// ══════════════════════════════════════════════════════════════════════════
// Platform abstraction: Serial.printf on Arduino, no-op on ESP-IDF
// ══════════════════════════════════════════════════════════════════════════

#ifdef PHOENIX_PLATFORM_ARDUINO
  #define PHOENIX_SERIAL_PRINT(fmt, ...) Serial.printf(fmt, ##__VA_ARGS__)
#else
  #define PHOENIX_SERIAL_PRINT(fmt, ...) do {} while(0)
#endif
...
static void check_float(float actual, float expected, float tolerance, const char* name) {
    ...
    if (ok) {
        ...
        PHOENIX_SERIAL_PRINT("FAIL: %s = %.4f (expected %.4f)\n", name,
                             (double)actual, (double)expected);
    }
}
...
// ══════════════════════════════════════════════════════════════════════════
// SECTION 9: Main Task + Entry Points
// ══════════════════════════════════════════════════════════════════════════
...
static void phoenix_main_task(void* /*arg*/) {
    ...
    PHOENIX_SERIAL_PRINT(">> Before NVS test, passed=%d failed=%d\n", tests_passed, tests_failed);
    test_nvs_storage();
    PHOENIX_SERIAL_PRINT(">> After NVS test, passed=%d failed=%d\n", tests_passed, tests_failed);
    vTaskDelay(pdMS_TO_TICKS(100));

    PHOENIX_SERIAL_PRINT(">> Before Safety test\n");
    test_safety_checks();
    PHOENIX_SERIAL_PRINT(">> After Safety test, passed=%d failed=%d\n", tests_passed, tests_failed);
...
// Arduino entry point (used by both PlatformIO and ESP-IDF+Arduino builds)
inline void setup() {
    Serial.begin(115200);
    delay(500);
