...
//       return Ok();
//   }
// Err from ErrorInfo (needed by PHOENIX_TRY for type-safe error propagation)
template <typename T>
Result<T> Err(ErrorInfo info) { return Result<T>::Err(info); }

inline Result<void> Err(ErrorInfo info) { return Result<void>::Err(info); }

#define PHOENIX_TRY(expr)                                    \
    do {                                                     \
        auto _phoenix_try_r = (expr);                        \
        if (_phoenix_try_r.is_err())                         \
            return ::phoenix::Err(_phoenix_try_r.error());   \
    } while (0)

#define PHOENIX_TRY_VAL(var, expr)                           \
    auto _phoenix_try_##var = (expr);                        \
    if (_phoenix_try_##var.is_err())                         \
        return ::phoenix::Err(_phoenix_try_##var.error());   \
    auto var = _phoenix_try_##var.value()
